...
import { api } from '../api';
import './Chat.css'; // Újrahasználjuk a chat stílusait
...
const Notifications: React.FC<NotificationsProps> = ({ userId, onRead }) => {
  ...
  useEffect(() => {
    fetchNotifications();
    // Amikor megnyitják, jelöljük mindet olvasottnak egy kis idő után
    ...
    <div className="chat-container">
      <div className="chat-messages">
        {notifications.length === 0 ? (
          <div style={{textAlign: 'center', padding: '20px', color: '#888'}}>Nincsenek értesítéseid.</div>
        ) : (
